...
void CImInspector::Show_Create_Object()
{
	...
	if(ImGui::Button("CREATE!"))
	{
		...
		if (0 == iObjNum)
		{
			...
			m_pCurRoom->PushBack_GameObj(LAYER_MONSTER, L"Baller" ,pGameObject, COL_ENEMY);
		}
		if (1 == iObjNum)
		{
			pName = "Bub";
			pGameObject = CBub::Create(m_pGraphicDev, m_vObjectPos);
			m_pCurRoom->PushBack_GameObj(LAYER_MONSTER, L"Bub", pGameObject, COL_ENEMY);
		}
		...
		if (2 == iObjNum)
		{
			...
			m_pCurRoom->PushBack_GameObj(LAYER_MONSTER, L"Guppi", pGameObject, COL_ENEMY);
		}
		if (3 == iObjNum)
		{
			pName = "Turret";
			pGameObject = CTurret::Create(m_pGraphicDev, m_vObjectPos);
			m_pCurRoom->PushBack_GameObj(LAYER_MONSTER, L"Turret", pGameObject, COL_ENEMY);
		}
		if (4 == iObjNum)
		{
			pName = "Walker";
			pGameObject = CWalker::Create(m_pGraphicDev, m_vObjectPos);
			m_pCurRoom->PushBack_GameObj(LAYER_MONSTER, L"Walker", pGameObject, COL_ENEMY);
		}
