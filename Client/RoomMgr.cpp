...
void CRoomMgr::Create_Default_Room()
{
	...
	CWormHead* pHead = CWormHead::Create(m_pGraphicDev, _vec3(75.f, 2.f, 15.f));
	m_arrRoom[3]->PushBack_GameObj(pHead);

	m_arrRoom[0]->Set_DoorType(DOOR_NE);
	m_arrRoom[1]->Set_DoorType(DOOR_NW);
	m_arrRoom[3]->Set_DoorType(DOOR_ES);
	m_arrRoom[4]->Set_DoorType(DOOR_SW);

	m_arrRoom[0]->PushBack_GameObj(CWalkerBoss::Create(m_pGraphicDev, _vec3(40.f, 3.f, 40.f)));
	m_arrRoom[1]->PushBack_GameObj(CNubBoss::Create(m_pGraphicDev, _vec3(100.f, 3.f, 40.f)));
